  ...
  <main class="create-startup-page">
    <div class="startup-card">
      ...
      <form class="startup-form" @submit.prevent="submitStartup">
        ...
        <input type="submit" value="Створити" :disabled="isSubmitting" />
      </form>
    </div>

    <div v-if="showModal" class="create-startup-modal-overlay" @click.self="cancelModalAction">
      <div class="create-startup-modal-content">
        <div class="create-startup-modal-header">
          <h4>
            <svg :class="['create-startup-modal-header-icon', modalConfig.type]" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="create-startup-modal-actions">
          <button v-if="!modalConfig.isNotification" @click="cancelModalAction" class="create-startup-modal-button cancel">{{ modalConfig.cancelButtonText }}</button>
          <button @click="proceedWithModalAction" :class="['create-startup-modal-button', modalConfig.isNotification ? (modalConfig.type === 'error' ? 'error-ok' : 'ok') : 'confirm']">{{ modalConfig.confirmButtonText }}</button>
        </div>
      </div>
    </div>
  </main>
...
<script setup>
import { ref, onMounted, onUnmounted } from 'vue'; // Додано onMounted, onUnmounted
...
const handleGlobalEscCreateStartup = (event) => {
...
const submitStartup = async () => {
  ...
  if (!jwt) {
    showNotificationModal('Помилка авторизації', 'Будь ласка, увійдіть до системи.', 'error', () => {
      router.push('/login');
    });
...
onMounted(() => {
  ...
  document.addEventListener('keyup', handleGlobalEscCreateStartup);
});

onUnmounted(() => {
  document.removeEventListener('keyup', handleGlobalEscCreateStartup);
});
...
.input-group textarea {
  min-height: 120px; /* Збільшена мінімальна висота для опису стартапу */
}
...
.error-message { /* Для помилок валідації форми, якщо вони відображаються окремо */
  color: #ffc2c2;
...
.create-startup-modal-overlay { /* Унікальний префікс */
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  ...
  z-index: 1050;
  opacity: 0;
  animation: fadeInOverlayCreateStartup 0.2s forwards;
}

@keyframes fadeInOverlayCreateStartup {
  to { opacity: 1; }
}
...
.create-startup-modal-content {
  ...
  opacity: 0;
  animation: scaleInModalCreateStartup 0.25s forwards cubic-bezier(0.175, 0.885, 0.32, 1.275);
}
@keyframes scaleInModalCreateStartup {
  to {
...
.create-startup-modal-header {
  display: flex;
  align-items: center;
  margin-bottom: 15px;
}
.create-startup-modal-header-icon {
  margin-right: 12px;
  flex-shrink: 0;
  width: 22px;
  height: 22px;
}
.create-startup-modal-header-icon.confirm {
  color: #f0b232;
}
.create-startup-modal-header-icon.success {
  color: #28a745;
}
.create-startup-modal-header-icon.error {
  color: #d83c3e;
}

.create-startup-modal-content h4 {
  margin:0;
  font-size: 1.2em;
  color: #ffffff;
  font-weight: 600;
...
.create-startup-modal-content p {
  margin-bottom: 25px;
  font-size: 1em;
  line-height: 1.65;
  color: #b9bbbe;
...
.create-startup-modal-actions {
  display: flex;
  justify-content: flex-end;
  gap: 12px;
}

.create-startup-modal-button {
  padding: 10px 20px;
  border: none;
  border-radius: 5px;
  font-size: 0.95em;
...
.create-startup-modal-button:active {
  transform: translateY(1px);
}

.create-startup-modal-button.confirm {
  background-color: #d83c3e;
  color: white;
  box-shadow: 0 2px 3px rgba(0,0,0,0.2);
}
.create-startup-modal-button.confirm:hover {
  background-color: #bf3032;
  box-shadow: 0 3px 5px rgba(0,0,0,0.25);
}

.create-startup-modal-button.cancel {
  background-color: #4f545c;
  color: #dcddde;
}
.create-startup-modal-button.cancel:hover {
  background-color: #5c626a;
}

.create-startup-modal-button.ok {
  background-color: #007bff;
  color: white;
}
.create-startup-modal-button.ok:hover {
  background-color: #0056b3;
}
.create-startup-modal-button.error-ok {
  background-color: #d83c3e;
}
.create-startup-modal-button.error-ok:hover {
  background-color: #bf3032;
}
</style>